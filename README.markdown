...
基本的には/usr/libexec/git-coreのままでいいはずです。
...
この機能は更新の仕組みの外にありますが、このスクリプトが変更される可能性はないと考えていいでしょう。
